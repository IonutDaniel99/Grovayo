...
class FollowingContent extends Component
{
    ...
    public function render()
    {
        ...
        $follow_model = User_Follow::where("user_follow_id", $username_id)->where('user_follow_status', 2)->get();

        $follower_list = [];
        if ($follow_model->isEmpty()) {
            return view('livewire.www.user.view-user.following-content', ['is_follower_list_empty' => true]);
        } else {
        ...
        return view('livewire.www.user.view-user.following-content', compact('follower_list', 'is_follower_list_empty'));
    }
}
